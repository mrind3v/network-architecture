...
# 49. Pointer tagging in nginx

The slide showed nginx storing the generation marker in the low bit of a pointer.

Why is that possible?

Because structures have alignment requirements.

A valid pointer to an aligned structure may look like:
...
Its lowest bit is guaranteed to be zero.

nginx borrows that otherwise-unused bit.
...
This technique is called pointer tagging.

---

# 50. Extracting the instance bit
...
# 51. Recovering the real pointer
...
`~1` has all bits set except the lowest bit.
...
Now nginx has the true aligned pointer again.

---

# 52. The stale-event check
...
means the connection object is no longer active/has been closed.
...
means the event's generation does not match the current connection generation.

Therefore:

```text
event belongs to old connection lifetime
→ ignore
```

---

# 53. Why only one bit can work

The bit toggles between:
...
It is not intended to be a globally unique connection ID.

It distinguishes the immediately relevant reuse generations according to nginx's event lifecycle.

Another implementation could use an explicit generation counter.

The slide mentions that a teaching/example implementation may use a normal counter instead.

nginx uses a bit because it is extremely cheap.

---

# 54. “Free bit, no extra memory, one branch”

That phrase summarizes the engineering trick.

The pointer already has an unused low bit due to alignment.

nginx uses it instead of adding:

```c
uint64_t generation;
```

So the check costs essentially:

```text
one bit of existing pointer representation
+
one comparison/branch
```

It is a compact systems-level optimization.
...
# 56. Full historical architecture comparison

This is worth memorizing.

| Architecture | What is reused? | What remains expensive? | Main idea |
|---|---|---|---|
| CGI | almost nothing | process/runtime startup per request | simple but expensive |
| Apache prefork | processes | one process per active connection | fork workers ahead of time |
| Apache worker MPM | processes + threads | thread tied to connection | threads cheaper than processes |
| FastCGI | application processes/runtime | worker occupation | persistent app workers |
| Java servlets | JVM, classes, servlet objects, thread pool | threads during processing | long-lived objects, method call per request |
| Apache event MPM | worker pool + event waiting | active request work | do not waste threads on idle keep-alives |
| nginx | fixed workers + event loops | active event handling | many connections per worker |
| epoll | persistent kernel watch state | ready-event processing | cost tracks readiness more closely |
...
# 60. Blocking vs nonblocking I/O
...
no data
↓
thread/process sleeps here
```

Nonblocking/event-driven approach:
...
↓
if more data needed:
register/wait for readiness
↓
return to event loop
```

The latter is how one execution worker can multiplex many sockets.
...
# 68. The single unifying story

If you forget details during the exam, reconstruct the history using this question:

> What are we paying for while no useful work is happening?

CGI:

```text
paying repeatedly for process/runtime startup
```

Fix:

```text
reuse runtime/application
```

Prefork:

```text
paying fork cost when demand arrives
```
...
```text
process is too heavy per connection
```
...
```text
idle connections still consume threads
```
...
```text
idle sockets still contribute to repeated scanning cost
```

Fix:

```text
persistent kernel watch set + ready-event queue via epoll
```

Multiple workers:

```text
workers can compete unnecessarily
```

Fix:

```text
local backpressure + kernel-assisted accept distribution
```

Resource reuse:

```text
old events can refer to recycled resources
```
...
That progression is the conceptual backbone of every slide we have covered.